...
const create = P.coroutine(function * (opts) {
  ...
  console.log(opts.files.join(`\n`))

  const audio1 = yield extractAudio(opts.files[0])
  let finalAudio = audio1
  if (opts.music) {
    console.log('Using music track:', opts.music)
    finalAudio = yield mixAudio(audio1.outfile, opts.music)
  }
